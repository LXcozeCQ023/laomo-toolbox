...
const { chooseImages: chooseDouyinImages } = require('./scripts/douyin-anonymous-resolver');
...
function safeFileName(value, fallback = '抖音视频') {
...
function findDouyinItem(value, id) {
  ...
  // 图文作品没有 video 字段，只有 images / image_list。
  if (String(value.aweme_id || '') === String(id) && (value.video || value.images || value.image_list)) return value;
...
async function getDouyinVideoInfo(originalUrl) {
  ...
  try {
    ...
    if (!item) throw new Error('抖音游客页面暂未提供这个作品的数据');
    ...
    // 图文作品没有播放地址，改为交出图集原图清单。
    const images = chooseDouyinImages(item);
    if (images.length) {
      return {
        kind: 'images',
        id,
    ...
    throw new Error('抖音游客页面暂未提供这个视频的播放地址');
  } catch (error) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        let images = [];
        if (kind === 'images') {
          images = (Array.isArray(data.images) ? data.images : []).map(String).filter((item) => isDouyinMediaUrl(item));
          if (!images.length) throw new Error('返回的图文信息不匹配');
        } else {
          if (!isDouyinMediaUrl(data.url)) throw new Error('返回的视频信息不匹配');
          url = data.url;
        }
        finish(null, {
          kind,
...
// 图文作品：下载图集原图到独立子目录，逐张校验文件头和结尾标记。
function isImageBytes(bytes) {
  ...
  if (head4 === '\x89PNG') return true;
  if (head4 === 'RIFF' && bytes.subarray(8, 12).toString('latin1') === 'WEBP') return true;
  if (head4 === 'GIF8') return true;
  return /^ftyp(heic|heix|hevc|mif1|msf1|avif)/.test(bytes.subarray(4, 12).toString('latin1'));
}
...
// 抖音图集常见 webp，转成 jpg 以便在剪映、PPT 等工具里直接使用；转码失败则保留原图。
function convertToJpeg(src, dest) {
  return new Promise((resolve, reject) => {
    const child = spawn(FFMPEG, ['-hide_banner', '-loglevel', 'error', '-y', '-i', src, '-q:v', '2', dest], { windowsHide: true });
    child.on('error', reject);
    child.on('close', (code) => (code === 0 ? resolve() : reject(new Error('图片转码失败'))));
  });
}
  ...
  try {
    ...
    try {
      response = await fetch(imageUrl, {
        ...
        signal: controller.signal,
        headers: {
          'User-Agent': info.userAgent || DOUYIN_MOBILE_UA,
    ...
    if (!isImageBytes(bytes)) throw new Error('图片内容不完整或格式无法识别');
    const file = path.join(dir, `${String(index).padStart(2, '0')}${imageExtension(contentType)}`);
    fs.writeFileSync(file, bytes, { flag: 'wx' });
    if (path.extname(file) === '.webp' && FFMPEG) {
      const jpg = file.slice(0, -'.webp'.length) + '.jpg';
      try {
        await convertToJpeg(file, jpg);
        fs.unlinkSync(file);
        return jpg;
      } catch {
        try { fs.unlinkSync(jpg); } catch {}   // 转码失败保留可用的 webp 原图
      }
    }
    return file;
  } finally {
...
<title>老默工具箱 · 视频下载器 · Mac 预览版</title>
<style>
...
html[data-theme="dark"]{color-scheme:dark;--bg:#050507;--surface:rgba(28,28,30,.72);--surface-solid:#1c1c1e;--field:#242426;--line:rgba(255,255,255,.09);--line-strong:rgba(255,255,255,.17);--tx:#f5f5f7;--tx2:#a1a1a6;--tx3:#6e6e73;--blue:#0a84ff;--blue-hover:#2490ff;--red:#ff453a;--green:#30d158;--shadow:0 28px 80px rgba(0,0,0,.42)}
html[data-theme="dark"] body{background:radial-gradient(circle at 50% -18%,rgba(10,132,255,.18),transparent 40%),var(--bg)}
html[data-theme="dark"] .platforms span,html[data-theme="dark"] .badge,html[data-theme="dark"] .foot,html[data-theme="dark"] .themerow button{background:rgba(28,28,30,.42)}
.themerow{display:flex;justify-content:center;margin-top:16px}
.themerow button{border-radius:999px;padding:7px 15px;font-size:12px;color:var(--tx2)}
...
</style><script nonce="${PAGE_NONCE}">try{var t=localStorage.getItem('toolbox-theme');document.documentElement.dataset.theme=t||(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light');}catch(e){document.documentElement.dataset.theme='light';}</script></head><body><div class="wrap">
<header class="hero">
...
function syncThemeBtn(){var b=$('#themeToggle');if(b)b.textContent=document.documentElement.dataset.theme==='dark'?'切换为亮色':'切换为暗色';}
$('#themeToggle').onclick=function(){var next=document.documentElement.dataset.theme==='dark'?'light':'dark';document.documentElement.dataset.theme=next;try{localStorage.setItem('toolbox-theme',next);}catch(e){}syncThemeBtn();};
syncThemeBtn();
refreshConf();tick();
